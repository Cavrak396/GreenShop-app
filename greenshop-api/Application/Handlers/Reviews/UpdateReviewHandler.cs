...
namespace greenshop_api.Application.Handlers.Reviews
{
    public class UpdateReviewHandler(
        ...
        IMapper mapper) : IRequestHandler<UpdateReviewCommand, Unit>
    {
        ...
        public async Task<Unit> Handle(UpdateReviewCommand request, CancellationToken cancellationToken)
        {
            ...
            var review = await _reviewsRepository.GetReviewByIdsAsync(userId, request.PlantId!);
            var newReview = _mapper.Map<Review>(request.Review);

            await _reviewsRepository.UpdateReviewAsync(review!, newReview);
            return Unit.Value;
        }
    }
}
