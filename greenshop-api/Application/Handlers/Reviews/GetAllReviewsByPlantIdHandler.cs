...
namespace greenshop_api.Application.Handlers.Reviews
{
    public class GetAllReviewsByPlantIdHandler(
        ...
        IMapper mapper) : IRequestHandler<GetAllReviewsByPlantIdQuery, List<GetReviewDto>>
    {
        ...
        public async Task<List<GetReviewDto>> Handle(GetAllReviewsByPlantIdQuery request, CancellationToken cancellationToken)
        {
            var reviewsQuery = _reviewsRepository.GetAllReviewsQueryable(request.PlantId!);

            reviewsQuery = reviewsQuery.Skip((request.Page - 1) * request.PageSize).Take(request.PageSize);

            var reviews = await reviewsQuery.ToListAsync(cancellationToken: cancellationToken);

            var userIds = reviews.Select(r => r.UserId).ToList();
            var users = await _usersRepository.GetUsersByIdsAsync(userIds!);

            var getReviewDtos = _mapper.Map<List<GetReviewDto>>(reviews);
